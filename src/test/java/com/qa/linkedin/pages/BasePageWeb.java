...
public class BasePageWeb extends TestBase
{
	...
	private static Logger log = Logger.getLogger(BasePageWeb.class);
	...
	void clickUsingJsExecutor(WebElement element) throws InterruptedException {
		wait.until(ExpectedConditions.elementToBeClickable(element));
		JavascriptExecutor executor = (JavascriptExecutor) driver;
	}

	boolean isClickable(WebElement element) throws InterruptedException {
		wait.until(ExpectedConditions.elementToBeClickable(element));
	...
	void clearTextWithBackSapce(WebElement element) throws InterruptedException {
		wait.until(ExpectedConditions.elementToBeClickable(element));
	...
	public boolean isCkeckedElement(WebElement element) throws InterruptedException {

		wait.until(ExpectedConditions.invisibilityOf(element));
//    	System.out.println(element.getAttribute("ng-reflect-model").toString());
		if (element.getAttribute("ng-reflect-model").toString().contentEquals("true")) {
			return true;
		}
		return false;
	}
	...
	void assertText(WebElement element, String text) throws InterruptedException {
		wait.until(ExpectedConditions.visibilityOf(element));
		Assert.assertEquals(element.getText().toLowerCase(),text.toLowerCase(), "The text "+text+" is not equals to "+element.getText());
	...
	void assertContainsTextByAttribute(WebElement element, String attribute, String text) throws InterruptedException {
		wait.until(ExpectedConditions.visibilityOf(element));
		Assert.assertTrue(element.getAttribute(attribute).toString().contains(text),
				"The text " + text + " does not contains the text " + element.getAttribute(attribute).toString());
	}
	...
	boolean assertTextFromListByText(List<WebElement> elements, String text) throws InterruptedException {
		...
		Assert.assertTrue(false, "The value [" + text + "] is not in the list.");
		return false;
	}
